...
contract myungwooNFT is KIP17Full("The Babel", "BB"), KIP17Mintable, KIP17Burnable, KIP17Pausable {

    string baseURI = "";
    string metadataFormat = ".json";
    string queryString = "?alt=media";
    uint256 totalNum = 0;
    ...
    modifier onlyOwner(){
        require(owner == msg.sender, "MinterRole: caller is not the owner of the contract");
        _;
    }
    ...
    function setQueryString(string memory _queryString) public onlyOwner {
        queryString = _queryString;
    }
    
    function mint(address receiver_address) public payable returns (bool) {
        _mint(receiver_address, ++totalNum);
        return true;
    }
    ...
    function tokenURI(uint256 tokenId) public view returns (string memory) {
        ...
        return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, tokenIdStr, metadataFormat, queryString )) : "";
    }
}
